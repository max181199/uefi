...
//  InitializeLib(ImageHandle, SystemTable); //XXX removed
...
*/
  ...
  for(UINTN i=0; i < gST->NumberOfTableEntries; i++)
  {
    Print(L"Table %llu GUID: %08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x\r\n", i,
            gST->ConfigurationTable[i].VendorGuid.Data1,
            gST->ConfigurationTable[i].VendorGuid.Data2,
            gST->ConfigurationTable[i].VendorGuid.Data3,
            gST->ConfigurationTable[i].VendorGuid.Data4[0],
            gST->ConfigurationTable[i].VendorGuid.Data4[1],
            gST->ConfigurationTable[i].VendorGuid.Data4[2],
            gST->ConfigurationTable[i].VendorGuid.Data4[3],
            gST->ConfigurationTable[i].VendorGuid.Data4[4],
            gST->ConfigurationTable[i].VendorGuid.Data4[5],
            gST->ConfigurationTable[i].VendorGuid.Data4[6],
            gST->ConfigurationTable[i].VendorGuid.Data4[7]);

    if (compare(&gST->ConfigurationTable[i].VendorGuid, &Acpi20TableGuid, 16))
    {
  ...
  if(!RSDPfound)
  {
    ...
    {
      if (compare(&gST->ConfigurationTable[i].VendorGuid, &AcpiTableGuid, 16))
      {
  ...
  GPU_CONFIG *Graphics;
  Status = gST->BootServices->AllocatePool(EfiLoaderData, sizeof(GPU_CONFIG), (void**)&Graphics);
  if(EFI_ERROR(Status))
  {
  ...
  Print(L"Data at RSDP (first 16 bytes): 0x%016llx%016llx\r\n", *(EFI_PHYSICAL_ADDRESS*)(gST->ConfigurationTable[RSDP_index].VendorTable + 8), *(EFI_PHYSICAL_ADDRESS*)gST->ConfigurationTable[RSDP_index].VendorTable);
#endif

  // Load a program and exit boot services, then pass a loader block to that program's entry point to execute the program
  Status = GoTime(ImageHandle, Graphics, gST->ConfigurationTable, gST->NumberOfTableEntries, gST->Hdr.Revision);
